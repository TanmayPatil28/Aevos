...
export default function AgentTerminal() {
  ...
  return (
    <div className="w-full bg-[#0d0d0d] rounded-xl border border-white/10 shadow-[inset_0_0_20px_rgba(0,0,0,0.5)] overflow-hidden font-mono text-[12px] h-[200px] flex flex-col">
      <div className="flex items-center gap-2 px-4 py-2 border-b border-white/[0.05] bg-[#111]">
        ...
        <span className="text-[#86868b] font-bold tracking-widest uppercase text-[10px]">Deep Dive Agent Terminal</span>
      </div>
      <div ref={scrollRef} className="flex-1 p-4 overflow-y-auto flex flex-col gap-1.5 custom-scrollbar scroll-smooth">
        {logs.map((log, i) => (
          <motion.div
          ...
          >
            ...
            <span className={i === logs.length - 1 ? "text-[#32d74b] font-bold" : "text-white/80"}>{log}</span>
          </motion.div>
        ))}
        {logs.length < 10 && (
