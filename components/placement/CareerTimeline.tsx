...
export default function CareerTimeline({ timeline }: { timeline: TimelineStep[] }) {
  return (
    <div className="space-y-6">
      <h3 className="text-lg font-bold text-foreground mb-4">Your Career GPS</h3>
      <div className="relative border-l border-border-strong ml-3 space-y-6">
        {timeline.map((step, idx) => (
          ...
          >
            ...
            <div className={`absolute -left-3 top-1 bg-[#1D1D1F] rounded-full p-0.5 border ${
              ...
              "text-foreground/40 border-border-strong"
            }`}>
            ...
            <div className={`p-4 rounded-xl border ${
              step.status === "CURRENT" ? "bg-[#1D1D1F] border-indigo-500/30" : "bg-[#1D1D1F] border-border"
            }`}>
              <div className="flex justify-between items-start mb-2">
                <span className={`text-xs font-bold uppercase tracking-wider ${
                  step.status === "CURRENT" ? "text-indigo-300" : "text-foreground/50"
                ...
                <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${
                  ...
                  "bg-[#222224] text-foreground/40"
                }`}>{step.status}</span>
              </div>
              <h4 className={`font-semibold mb-2 ${
                step.status === "FUTURE" ? "text-foreground/70" : "text-foreground"
              }`}>{step.title}</h4>
