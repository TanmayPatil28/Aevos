...
export class SppuDocumentParser implements AcademicDocumentParser {
  supports(presetId: string): boolean {
    return presetId.toLowerCase() === "sppu";
  }
  ...
  parse(rawText: string): ParsedAcademicDocument {
    ...
    // Process semester by semester and current courses
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (inCurrentSemesterSection) {
        ...
        // Format: CS-201 Design & Analysis of Algorithms 4 CIE: 44 ATT: 40 BUNK: 2
        // Or simple: CS-201 Design & Analysis of Algorithms 4
        ...
        // Standard course: CS-101 Programming & Problem Solving 4 A
        // Let's support code name credits grade
        ...
        if (courseMatch) {
          ...
          if (semObj && semObj.courses) {
            ...
            // Accumulate credits dynamically if they weren't explicitly set yet
            if (semObj.credits.value === 0) {
              // We will sum them up at the end if not set, or let's do it dynamically
            }
          }
        }
      }
    ...
    // Fallback/post-process credits if not explicitly found in text
    semesterHistory.forEach(sem => {
      if (sem.credits.value === 0 && sem.courses && sem.courses.length > 0) {
        ...
        // Estimate earned credits (sum of credits for courses with passing grades)
        // In SPPU, 'F' is the fail grade
        const earnedCreds = sem.courses
          .filter(c => c.grade.value !== "F")
