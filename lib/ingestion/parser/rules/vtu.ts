...
export class VtuDocumentParser implements AcademicDocumentParser {
  supports(presetId: string): boolean {
    return presetId.toLowerCase() === "vtu";
  }
  ...
  parse(rawText: string): ParsedAcademicDocument {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (inCurrentSemesterSection) {
        ...
        // Format: CS31 Data Structures & Applications 4 CIE: 42 ATT: 44 BUNK: 3
        ...
        // Course matching
        // e.g. MATH11 Advanced Mathematics I 4 A
    ...
    semesterHistory.forEach(sem => {
      if (sem.credits.value === 0 && sem.courses && sem.courses.length > 0) {
        ...
        // In VTU, passing grades are anything that is NOT 'F' or 'NP'
        const earnedCreds = sem.courses
          .filter(c => c.grade.value !== "F" && c.grade.value !== "NP")
