...
export async function handleFetch({ id }, neonClient, extra) {
    try {
        // Parse the ID format
...
async function fetchOrganizationDetails(orgId, neonClient) {
    try {
        const { data: orgData } = await neonClient.getOrganization(orgId);
        let members = [];
        try {
            const { data: membersData } = await neonClient.getOrganizationMembers(orgId);
...
async function fetchProjectDetails(projectId, neonClient) {
    try {
...
async function fetchBranchDetails(projectId, branchId, neonClient, extra) {
    try {
