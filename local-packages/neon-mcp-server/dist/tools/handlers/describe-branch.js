...
export async function handleDescribeBranch({ projectId, databaseName, branchId, }, neonClient, extra) {
    const { data: branchData } = await neonClient.getProjectBranch(projectId, branchId);
    ...
    try {
        ...
        response = await runQuery.transaction(DESCRIBE_DATABASE_STATEMENTS.map((sql) => runQuery.query(sql)));
        return {
            content: [
                {
                    type: 'text',
