...
export async function handleSearch({ query }, neonClient, extra) {
    try {
        const results = [];
        ...
        if (!extra.account.isOrg) {
            ...
            const searchResults = await searchProjectsAndBranches(projects, neonClient, searchQuery);
        ...
        try {
            ...
            const searchResults = await searchProjectsAndBranches(data.projects, neonClient, searchQuery);
            results.push(...searchResults);
        }
        catch {
...
const searchProjectsAndBranches = async (projects, neonClient, query) => {
    const results = [];
...
const searchBranches = async (projectId, neonClient, query) => {
    try {
