 ...
 */
export async function getDefaultDatabase({ projectId, branchId, databaseName, }, neonClient) {
    const { data } = await neonClient.listProjectBranchDatabases(projectId, branchId);
    ...
    const defaultDatabase = databases.find((db) => db.name === NEON_DEFAULT_DATABASE_NAME);
 ...
 */
export async function getOrgByOrgIdOrDefault(params, neonClient, extra) {
    ...
    const consoleOrganizations = organizations.filter((org) => org.managed_by === 'console');
    ...
    const orgList = organizations
        .map((org) => `- ${org.name} (ID: ${org.id}) [managed by: ${org.managed_by}]`)
        .join('\n');
    throw new NotFoundError(`Multiple organizations found. Please specify the org_id parameter with one of the following organization IDs:\n${orgList}`);
}
export function filterOrganizations(organizations, search) {
    if (!search) {
 ...
 */
export function looksLikeBranchId(branch) {
 ...
 */
export async function resolveBranchId(branchNameOrId, projectId, neonClient) {
    ...
    if (looksLikeBranchId(branchNameOrId)) {
        ...
        if (!branch) {
            throw new NotFoundError(`Branch ID "${branchNameOrId}" not found in project ${projectId}`);
        }
    ...
    if (!branch) {
        ...
        throw new NotFoundError(`Branch name "${branchNameOrId}" not found in project ${projectId}.\nAvailable branches: ${availableBranches}`);
    }
