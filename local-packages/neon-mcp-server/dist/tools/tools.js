...
import { DESCRIBE_DATABASE_STATEMENTS, getDefaultDatabase, splitSqlStatements, getOrgByOrgIdOrDefault, filterOrganizations, resolveBranchId, } from './utils.js';
import { startSpan } from '@sentry/node';
async function handleListProjects(params, neonClient, extra) {
...
async function handleCreateProject(params, neonClient) {
...
async function handleDeleteProject(projectId, neonClient) {
...
async function handleRunSql({ sql, databaseName, projectId, branchId, }, neonClient, extra) {
...
async function handleRunSqlTransaction({ sqlStatements, databaseName, projectId, branchId, }, neonClient, extra) {
    const connectionString = await handleGetConnectionString({
        projectId,
        branchId,
        databaseName,
...
async function handleGetDatabaseTables({ projectId, databaseName, branchId, }, neonClient, extra) {
    const connectionString = await handleGetConnectionString({
        projectId,
        branchId,
        databaseName,
...
async function handleDescribeTableSchema({ projectId, databaseName, branchId, tableName, }, neonClient, extra) {
    const connectionString = await handleGetConnectionString({
        projectId,
        branchId,
        databaseName,
...
async function handleCreateBranch({ projectId, branchName, }, neonClient) {
...
async function handleDeleteBranch({ projectId, branchId, }, neonClient) {
    const response = await neonClient.deleteProjectBranch(projectId, branchId);
    return response.data;
}
async function handleResetFromParent({ projectId, branchIdOrName, preserveUnderName, }, neonClient) {
    // Resolve branch name or ID to actual branch ID and get all branches in one call
    const { branchId: resolvedBranchId, branches } = await resolveBranchId(branchIdOrName, projectId, neonClient);
    const branch = branches.find((b) => b.id === resolvedBranchId);
    if (!branch) {
        throw new NotFoundError(`Branch "${branchIdOrName}" not found in project ${projectId}`);
    }
    ...
    const parentBranch = branch.parent_id
        ...
        : undefined;
    if (!parentBranch) {
        throw new InvalidArgumentError(`Branch "${branchIdOrName}" does not have a parent branch and cannot be reset`);
    }
    ...
    const response = await neonClient.restoreProjectBranch(projectId, resolvedBranchId, {
...
async function handleGetConnectionString({ projectId, branchId, computeId, databaseName, roleName, }, neonClient, extra) {
    return await startSpan({
        ...
        if (!projectId) {
            ...
            if (projects.length === 1) {
                ...
                throw new NotFoundError('Please provide a project ID or ensure you have only one project in your account.');
            }
        }
        if (!branchId) {
            ...
            if (defaultBranch) {
                ...
                throw new NotFoundError('No default branch found in this project. Please provide a branch ID.');
            }
        }
        ...
        if (!databaseName) {
            ...
            const { data } = await neonClient.getProjectBranchDatabase(projectId, branchId, databaseName);
...
async function handleSchemaMigration({ migrationSql, databaseName, projectId, }, neonClient, extra) {
...
async function handleCommitMigration({ migrationId }, neonClient, extra) {
...
async function handleDescribeBranch({ projectId, databaseName, branchId, }, neonClient, extra) {
    const connectionString = await handleGetConnectionString({
        projectId,
        branchId,
        databaseName,
...
async function handleExplainSqlStatement({ params, }, neonClient, extra) {
...
async function createTemporaryBranch(projectId, neonClient) {
...
async function handleQueryTuning(params, neonClient, extra) {
    let tempBranch;
    ...
    try {
        ...
        if (tableNames.length === 0) {
            throw new NotFoundError('No tables found in execution plan. Cannot proceed with optimization.');
        }
        // Get schema information for all referenced tables in parallel
        const tableSchemas = await Promise.all(tableNames.map(async (tableName) => {
            try {
...
function extractExecutionMetrics(plan) {
    try {
...
function extractTableNamesFromPlan(planResult) {
    const tableNames = new Set();
    function recursivelyExtractFromNode(node) {
        if (!node || typeof node !== 'object')
            return;
...
async function handleCompleteTuning(params, neonClient, extra) {
    let results;
    const operationLog = [];
    try {
        ...
        const result = {
            appliedChanges: params.applyChanges && params.suggestedSqlStatements
                ? params.suggestedSqlStatements
                : undefined,
            results,
            deletedBranches: params.shouldDeleteTemporaryBranch && params.temporaryBranch
...
async function handleListSlowQueries({ projectId, branchId, databaseName, computeId, limit = 10, }, neonClient, extra) {
  ...
  `;
    ...
    if (!extensionExists) {
        throw new NotFoundError(`pg_stat_statements extension is not installed on the database. Please install it using the following command: CREATE EXTENSION pg_stat_statements;`);
    }
...
async function handleListBranchComputes({ projectId, branchId, }, neonClient, extra) {
    // If projectId is not provided, get the first project but only if there is only one project
    if (!projectId) {
        const projects = await handleListProjects({}, neonClient, extra);
        if (projects.length === 1) {
            ...
            throw new InvalidArgumentError('Please provide a project ID or ensure you have only one project in your account.');
        }
    }
    let endpoints;
    if (branchId) {
        const response = await neonClient.listProjectBranchEndpoints(projectId, branchId);
    ...
    return endpoints.map((endpoint) => ({
        ...
        compute_size: endpoint.autoscaling_limit_min_cu !== endpoint.autoscaling_limit_max_cu
...
async function handleListOrganizations(neonClient, account, search) {
    if (account.isOrg) {
...
async function handleListSharedProjects(params, neonClient) {
...
export const NEON_HANDLERS = {
    list_projects: async ({ params }, neonClient, extra) => {
        const organization = await getOrgByOrgIdOrDefault(params, neonClient, extra);
        const projects = await handleListProjects({ ...params, org_id: organization?.id }, neonClient, extra);
        return {
            content: [
                {
                    type: 'text',
    ...
    create_project: async ({ params }, neonClient, extra) => {
        try {
            const organization = await getOrgByOrgIdOrDefault(params, neonClient, extra);
            const result = await handleCreateProject({ project: { name: params.name, org_id: organization?.id } }, neonClient);
    ...
    describe_project: async ({ params }, neonClient) => {
        ...
        return {
            content: [
                ...
                {
                    type: 'text',
                    text: `It contains the following branches (use the describe branch tool to learn more about each branch): ${JSON.stringify(result.branches, null, 2)}`,
                },
            ],
        };
    },
    ...
    reset_from_parent: async ({ params }, neonClient) => {
        ...
        if (result.preservedBranchName) {
            messages.push(params.preserveUnderName
    ...
    explain_sql_statement: async ({ params }, neonClient, extra) => {
        const result = await handleExplainSqlStatement({ params }, neonClient, extra);
        return result;
    },
    ...
    complete_query_tuning: async ({ params }, neonClient, extra) => {
        const result = await handleCompleteTuning({
            ...
            branch: params.branchId
                ? { id: params.branchId, project_id: params.projectId }
                : undefined,
        }, neonClient, extra);
        return {
            content: [
    ...
    list_organizations: async ({ params }, neonClient, extra) => {
        const organizations = await handleListOrganizations(neonClient, extra.account, params.search);
        return {
            content: [
                {
                    type: 'text',
