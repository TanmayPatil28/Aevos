import { discovery, buildAuthorizationUrl, authorizationCodeGrant, ClientSecretPost, refreshTokenGrant, } from 'openid-client';
import { CLIENT_ID, CLIENT_SECRET, UPSTREAM_OAUTH_HOST, REDIRECT_URI, SERVER_HOST, } from '../constants.js';
...
const getUpstreamConfig = async () => {
    ...
    const config = await discovery(url, CLIENT_ID, {
        client_secret: CLIENT_SECRET,
    }, ClientSecretPost(CLIENT_SECRET), {});
    return config;
};
export const upstreamAuth = async (state) => {
...
export const exchangeCode = async (req) => {
    try {
        const config = await getUpstreamConfig();
...
export const exchangeRefreshToken = async (token) => {
    const config = await getUpstreamConfig();
