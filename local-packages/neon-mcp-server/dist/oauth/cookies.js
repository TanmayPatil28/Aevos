 ...
 */
const verifySignature = async (key, signatureHex, data) => {
    try {
 ...
 */
const getApprovedClientsFromCookie = async (cookie, secret) => {
    if (!cookie)
        return [];
    try {
 ...
 */
export const isClientAlreadyApproved = async (req, clientId, cookieSecret) => {
    const approvedClients = await getApprovedClientsFromCookie(req.cookies[COOKIE_NAME] ?? '', cookieSecret);
 ...
 */
export const updateApprovedClientsCookie = async (req, res, clientId, cookieSecret) => {
    const approvedClients = await getApprovedClientsFromCookie(req.cookies[COOKIE_NAME] ?? '', cookieSecret);
    const newApprovedClients = JSON.stringify(Array.from(new Set([...approvedClients, clientId])));
