...
import { decodeAuthParams, extractClientCredentials, generateRandomString, parseAuthRequest, toMilliseconds, toSeconds, verifyPKCE, } from './utils.js';
...
import { isClientAlreadyApproved, updateApprovedClientsCookie, } from './cookies.js';
...
export const registerClient = async (req, res) => {
    ...
    if (payload.grant_types === undefined ||
        !payload.grant_types.every((grant) => SUPPORTED_GRANT_TYPES.includes(grant))) {
        res.status(400).json({
            code: 'invalid_request',
            error: 'grant_types is required and must only include supported grant types',
        });
        return;
    }
    if (payload.response_types === undefined ||
        !payload.response_types.every((responseType) => SUPPORTED_RESPONSE_TYPES.includes(responseType))) {
        res.status(400).json({
            code: 'invalid_request',
            error: 'response_types is required and must only include supported response types',
        });
        return;
    }
    try {
...
*/
authRouter.get('/authorize', bodyParser.urlencoded({ extended: true }), async (req, res) => {
...
authRouter.post('/authorize', bodyParser.urlencoded({ extended: true }), async (req, res) => {
    ...
    await updateApprovedClientsCookie(req, res, requestParams.clientId, COOKIE_SECRET);
...
*/
authRouter.get('/callback', bodyParser.urlencoded({ extended: true }), async (req, res) => {
...
*/
authRouter.post('/token', bodyParser.urlencoded({ extended: true }), async (req, res) => {
    ...
    if (formData.grant_type === 'authorization_code') {
        ...
        if (isPkceEnabled &&
            !verifyPKCE(authorizationCode.code_challenge, authorizationCode.code_challenge_method, formData.code_verifier)) {
            res.status(400).json({
                code: 'invalid_grant',
        ...
        const providedRefreshToken = await model.getRefreshToken(formData.refresh_token);
        ...
        const upstreamToken = await exchangeRefreshToken(providedRefreshToken.refreshToken);
        const now = Date.now();
