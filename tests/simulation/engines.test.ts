...
function runEnginesTests() {
  ...
  assert(
    "Excellent student receives EXCELLENT health status",
    excellentHealth.status === "EXCELLENT",
  ...
  assert(
    "Student with active backlogs receives CRITICAL status immediately",
    backlogHealth.status === "CRITICAL",
  ...
  assert(
    "Student with low attendance (< 75%) receives CRITICAL status immediately",
    lowAttHealth.status === "CRITICAL",
