...
export default function TimelinePage() {
  ...
  const dynamicSemesters = React.useMemo(() => {
    ...
    // Find upcoming/current semesters safely
    ...
    // Collect all unique semesters from active courses that are > maxHistorySem, capped at 12 to prevent phantoms
    ...
    upcomingSems.forEach((semId, idx) => {
      sems.push({
        ...
        sgpa: 'TBD',
        focus: store.courses.filter(c => (c.semester || 1) === semId).map((c: any) => c.name).slice(0, 5),
        color: COLORS[(semId - 1) % COLORS.length]
      });
    });
  ...
  useEffect(() => {
    setMounted(true);
    if (mounted && !store.identity.hasAuthoritativeData) {
      router.replace('/dashboard');
    }
  }, [mounted, store.identity.hasAuthoritativeData, router]);

  if (!mounted) {
    return (
      <div className="min-h-screen bg-black pt-24 pb-32 max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 w-full">
        <div className="animate-pulse space-y-16">
          ...
          <div className="flex flex-col lg:flex-row gap-12">
            ...
            <div className="w-full lg:w-7/12">
              <div className="h-[500px] w-full bg-white/5 rounded-[2rem]" />
            </div>
          </div>
        </div>
      </div>
      ...
      <PageContainer className="pt-24 pb-32 max-w-[1200px] space-y-0 relative z-10">
        ...
        {dynamicSemesters.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-32 text-center">
            ...
            <h3 className="text-2xl font-bold text-white mb-2">No Academic History</h3>
            <p className="text-white/50 max-w-md mx-auto mb-8">
              We couldn't find any semester history to display on your timeline.
            </p>
            <Link href="/dashboard" className="px-6 py-3 bg-white text-black hover:bg-white/90 rounded-full font-bold transition-all">
              Return to Dashboard
            </Link>
          </div>
        ) : (
